<?php
   ...
   if(isset($_GET['id'])){
      ...
      if(buscarUnicoCliente($id)){
         ...
         $resultado = buscarUnicoCliente($id);
         return $resultado;

      }else{
        $msg = "<script> alert('[ Erro ]: item não encontrado no banco de dados.'); window.open('../view/cliente/visualizarCliente.php?id=$id', '_self') </script>";
        return $msg;
      }
   }else{
